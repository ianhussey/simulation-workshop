...
res <- lapply(X = cohens_d_values, FUN = interpret_cohens_d)
...
res <- map(cohens_d_values, interpret_cohens_d)
...
# apply function 
res <- dat |>
  mutate(interpretation = map(cohens_d, interpret_cohens_d)) |>
...
set.seed(42) # to generate the same data both times, we'll set the seed
